...
### DNS
DNS(Domain Name System)는 IP주소와 도메인을 매핑시켜 네트워크 통신 요청 시 요청 도메인에 해당하는 IP를 찾아주기 위한 시스템이다. 컴퓨터가 이 DNS를 활용하여 IP를 얻어오기 까지의 과정과 방법은 상당히 다양하다.
  
**DNS Server**  
- IP 설정에는 반드시 DNS 서버의 주소가 들어가야한다. DHCP를 사용하면 DHCP 서버가 할당해주는데, 다른 DNS 서버를 직접 지정하여 사용할 수도 있다.  

**Root DNS Server**  
- root DNS 서버는 DNS서버의 DNS서버와 같은 개념으로, 클라이언트가 DNS 서버에 도메인명으로 질의를 했으나 DNS서버에서 해당 도메인에 대한 정보를 찾지 못한 경우, DNS 서버는 Root DNS서버에 해당 정보가 있는지 질의를 한다.

**hosts**  
- 클라이언트가 항상 DNS 서버에 질의를 하는 것은 아니다. 가장 먼저 OS의 hosts라는 파일을 찾는데, 이는 도메인과 ip를 매핑해둔 파일로서 여기서 도메인 정보가 발견되면 dns서버 질의 없이 바로 해당 ip를 사용하게 된다.

**DNS cache**  
- 특정 도메인을 DNS 서버에 질의 하여 IP를 얻어온 경우, 운영체제는 이 IP를 1회성으로만 사용하지 않고 DNS 캐시에 저장한다. DNS 캐시는 몇시간 가량 유지되며, hosts 에서 도메인을 발견하지 못했을 시 DNS 서버로 보내기 전에 DNS 캐시를 먼저 확인하고 캐시에 있으면 해당 정보를 사용한다.


#### URL&URI
![img.png](../assets/URL&URI.png)  
URL과 URI는 명확하게 구분되는 다른 개념이라기보다는 비슷한 개념이지만 URI가 좀더 폭넓은 의미를 가진 용어라고 생각하면 된다.  
- **URL(Uniform Resource Locator)** : Locator, 즉 리소스의 위치를 나타내는 것으로, 보통 호스트와 도메인까지, 즉 "www.naver.com"까지를 URL의 개념으로 본다.
- **URI(Uniform Resource Identifier)** : Identifier, 즉 식별자라는 의미로, URL에 추가로 자원의 상세 식별자까지 포함한다. blog.naver.com 뒤의 "/user123/posts/24312" 와 같은 것까지 포함한 것을 의미한다.
